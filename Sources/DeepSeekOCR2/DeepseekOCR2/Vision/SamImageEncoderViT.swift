...
final class SamAttention: Module {
    ...
    private func computeRelPosBias(queries: MLXArray, height: Int, width: Int) -> MLXArray {
        ...
        let Rh = getRelPos(qSize: height, kSize: height, relPos: relPosH)
        let Rw = getRelPos(qSize: width, kSize: width, relPos: relPosW)
    ...
    private func getRelPos(qSize: Int, kSize: Int, relPos: MLXArray) -> MLXArray {
        ...
        if relPos.dim(0) != maxRelDist {
            ...
            relPosResized = Upsample(scaleFactor: [scale], mode: .linear(alignCorners: false))(relPos3D)
                .squeezed(axis: 0)
        }

        // Avoid CPU-side array construction in this hot path; build coordinates with MLX ops.
