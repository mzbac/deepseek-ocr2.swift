...
final class DeepseekV2MoEGate: Module {
    ...
    private var cachedWeightT: MLXArray?
    ...
    func callAsFunction(_ hiddenStates: MLXArray) -> (MLXArray, MLXArray) {
        ...
        let weightT: MLXArray
        if let cachedWeightT {
            weightT = cachedWeightT
        } else {
            let resolved = (weight.dtype == .float32 ? weight : weight.asType(.float32)).transposed(1, 0)
            cachedWeightT = resolved
            weightT = resolved
        }
        let logits = matmul(flat, weightT)
